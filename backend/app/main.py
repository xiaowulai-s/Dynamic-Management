...
# 导入路由
from app.routers import auth, equipment, logs, upload, analytics, settings as settings_router, notifications, export, customers, feedback, audit, backup, csv_io
...
# 创建FastAPI应用（S7: 非 DEBUG 模式下关闭文档）
app = FastAPI(
...
app.include_router(csv_io.router, tags=["CSV导入导出"])
...
async def global_exception_handler(request: Request, exc: Exception):
    """全局异常处理器（S8/Q28: 使用 logging，避免向客户端泄露内部细节）"""
