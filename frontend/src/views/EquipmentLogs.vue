  ...
  <div class="el-page">
    <div class="page-header">
      ...
      <div style="display:flex;align-items:center;gap:8px">
        <el-date-picker v-model="dateRange[0]" type="date" value-format="YYYY-MM-DD" placeholder="开始日期" style="width:150px" @change="fetchLogs" />
        <span style="color:var(--text-tertiary)">至</span>
        <el-date-picker v-model="dateRange[1]" type="date" value-format="YYYY-MM-DD" placeholder="结束日期" style="width:150px" @change="fetchLogs" />
        <el-button text @click="$router.back()">返回设备列表</el-button>
      </div>
    </div>

    <el-card shadow="never" v-loading="loading">
      <el-empty v-if="!logs.length" description="该设备暂无日志记录" />
      <div v-else class="timeline">
        <div v-for="log in logs" :key="log.id" class="tl-item">
          <div class="tl-dot" :class="dotClass(log.log_type)"></div>
          <div class="tl-card">
            <div class="tl-card-header">
              <el-tag :type="tagType(log.log_type)" size="small">{{ typeLabels[log.log_type] || log.log_type }}</el-tag>
              <el-tag :type="log.status==='approved'?'success':log.status==='rejected'?'danger':'warning'" size="small">{{ statusLabels[log.status] || log.status }}</el-tag>
            ...
            <div class="tl-card-body">
              ...
              <div v-if="log.status==='approved'||log.status==='rejected'" class="tl-approval">
                ...
                <div class="tl-row"><span class="tl-label">审批结果</span>
                  <el-tag :type="log.status==='approved'?'success':'danger'" size="small">{{ log.status==='approved'?'通过':'驳回' }}</el-tag>
                </div>
                <div v-if="log.rejection_reason" class="tl-row">
                  <span class="tl-label">驳回原因</span>
                  <span style="color:var(--color-danger-500)">{{ log.rejection_reason }}</span>
                </div>
              </div>
            </div>
          </div>
...
const tagType = (t: string) => ({ installation: 'info', repair: 'warning', scrap: 'danger', inspection: 'success', maintenance: 'success', fault: 'danger', parts: 'warning', calibration: 'info' } as any)[t] || 'info'
...
.tl-card-header { display: flex; align-items: center; gap: 8px; padding: 10px 14px; background: var(--surface-elevated); border-bottom: 1px solid var(--border-default); }
.tl-time { margin-left: auto; font-size: 12px; color: var(--text-tertiary); }
